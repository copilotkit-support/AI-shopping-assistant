...
async def chat_node(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    state["messages"].append(AIMessage(id=str(uuid.uuid4()), tool_calls=[{"name": "list_products", "args": {"products": (results_all[0]['products'])}, "id": str(uuid.uuid4())}], type="ai",  content=''))
    return Command(
        goto=END,
        update={
